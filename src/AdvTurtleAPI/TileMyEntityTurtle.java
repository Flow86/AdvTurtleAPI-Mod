...
public class TileMyEntityTurtle extends TileEntityTurtle {
	...
	private static final int[] xOffsetLD = { 0, 0, -1, 1, -1, 1 };
	private static final int[] yOffsetLD = { 0, 0, -1, -1, -1, -1 };
	private static final int[] zOffsetLD = { 0, 0, -1, 1, 1, -1 };
			...
			if (animation != null && animation.intValue() == -1) {
				...
				if (peek != null) {
					if (((Integer) peek).intValue() >= mod_AdvTurtleAPI.cmdOffset)
						...
						//System.out.println("Calling base command: " + ((Integer) peek).intValue());
						base = true;
					}
				}
				...
				case 5: // placeFrontLeftDown
					...
					flag = place(xOffsetLD[dir], yOffsetLD[dir], zOffsetLD[dir], getOppositeDir(dir));
	...
	private boolean place(int xOffset, int yOffset, int zOffset, int meta) {
		...
		//System.out.println("place: " + xOffset + "/" + yOffset + "/" + zOffset + ": " + itemstack.itemID + "@" + meta);

		if (worldObj.setBlockAndMetadataWithNotify(x, y, z, itemstack.itemID, item.getMetadata(itemstack.getItemDamage()))) {
